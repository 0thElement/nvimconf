...
-- One notification per completed request, never one per streamed token.
-- The full text is also retained in PiLog and mini.notify's history.
function M.show_response(session)
  if session.response_notified then return end
  local responses = vim.list_extend({}, session.responses or {})
  local partial = table.concat(session.response_chunks or {})
  if partial ~= '' then responses[#responses + 1] = partial end
  local text = table.concat(responses, '\n\n')
  if text == '' then return end
  session.response_notified = true
  local message = 'Pi response (full text: :PiLog)\n\n' .. text
  local mini = rawget(_G, 'MiniNotify')
  if type(mini) == 'table' and type(mini.add) == 'function' then
    local id = mini.add(message, 'INFO', 'Normal', { source = 'pi', session_id = session.id })
    vim.defer_fn(function() mini.remove(id) end, 15000)
  else
    vim.notify(message, vim.log.levels.INFO, { title = 'Pi response' })
  end
end
